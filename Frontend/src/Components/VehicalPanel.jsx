...
const VehicalPanel = (props) => {
  ...
  // console.log(props.ridedata?.rideData?.car?.totalFare)
  ...
  return (
    <div>
          ...
          <div onClick={()=>{
            ...
            }} className='px-2 flex items-center border-2 border-gray-100 active:border-black rounded-xl'>
            ...
            <div className=' p-1 w-full  leading-tight'>
              ...
              <h4>{`${((props.ridedata?.rideData?.car?.timeFare)/60).toFixed(2)} h`}</h4>
              <span className='text-[12px] text-gray-400'>Affordable, compact rides</span>
            </div>
            <h2 className='font-medium text-l p-1'>{`₹ ${props.ridedata?.rideData?.car?.totalFare}`}</h2>
          </div>

          <div onClick={()=>{props.setConfirmride(true)
             props.setvechileType("bike")}} className='px-2 gap-2  flex flex-row items-center border-2 border-gray-100 active:border-black rounded-xl'>
            ...
            <div className=' p-1 w-full  leading-tight'>
              ...
              <h4>{`${((props.ridedata?.rideData?.bike?.timeFare)/60).toFixed(2)} h`}</h4>
              <span className='text-[12px] text-gray-400'>Affordable, motercycle rides</span>
            </div>
            <h2 className='font-medium text-l p-1'>{`₹ ${props.ridedata?.rideData?.bike?.totalFare}`}</h2>
          </div>

          <div onClick={()=>{props.setConfirmride(true)
            props.setvechileType("auto")
          }}  className='px-1  flex flex-row items-center border-2 border-gray-100 active:border-black rounded-xl'>
            <img className='h-15 p-2' src="https://clipart-library.com/2023/Uber_Auto_312x208_pixels_Mobile.png" />
            <div className=' p-1 w-full  leading-tight'>
              <h2 className='font-medium'>UberGo <i className="ri-user-fill">4</i></h2>
              <h4>{`${((props.ridedata?.rideData?.auto?.timeFare)/60).toFixed(2)} h`}</h4>
              <span className='text-[12px] text-gray-400'>Affordable, compact rides</span>
            </div>
            <h2 className='font-medium text-l p-1 '>{`₹ ${props.ridedata?.rideData?.auto?.totalFare}`}</h2>
          </div>
    </div>
  )
}

export default React.memo(VehicalPanel)