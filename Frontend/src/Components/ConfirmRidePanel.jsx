...
const ConfirmRidePanel = (props) => {
  ...
  return (
    <div className='p-3 '>
      <h2 className='w-full flex justify-center '><i className=" text-gray-300 flex justify-center text-2xl ri-arrow-down-wide-line w-[80%] mb-4 " onClick={() => { props.setconfirmRidePanel(false) }}></i> </h2>
      <h1 className='text-2xl font-semibold  mb-6'>Confirm This Ride to Start</h1>
      ...
      <form onSubmit={(e) => { submitHandler(e) }} className='w-full mt-8'>
        <input
          type="number"
          value={otp}
          onChange={(e) => { setotp(e.target.value) }}
          placeholder='Enter Otp Here'
          className='w-full p-4 border-none active: border-black  rounded bg-[#eeee]' />

        <div className='flex flex-row justify-between items-center mt-6  w-full'>
          <button onClick={() => {
            props.setridePopUp(false)
            props.setconfirmRidePanel(false)
          }} className='text-lg text-center text-white bg-red-500 rounded w-25 p-2 ml-30'> Cancel</button>
          <Link to={'/captain-ride'} className='text-lg text-center text-white  bg-green-600  rounded w-25 p-2 active:scale-95 '>Confirm</Link>
        </div>
